    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "Check Users", callback_data=Response.CHECK_USERS.name
            ),
...
def keyboard_button_pressed(
    ...
    query.edit_message_text(
        text=f"You selected selected option: {query.data}"
    )  # for debugging, remove once final
    ...
    if query.data == Response.CHECK_USERS.name:
        ...
        return context.bot.send_message(
            chat_id=update.effective_chat.id, text="Nothing to see here."
        )
...
def help_command(update, context) -> None:
    ...
    context.bot.send_message(
        chat_id=update.effective_chat.id, text="Use /start to test this bot."
    )
